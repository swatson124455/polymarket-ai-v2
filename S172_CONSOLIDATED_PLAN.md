...
**Pacing-model recalibration — bias towards concrete-shippable-this-session.** Audit and forward-pacing recommendations should target what the next session can plausibly accomplish in one work-day, not what the next session should defer for safety. The S198–S202 chain produced five consecutive sessions where the predecessor handoff's recommended scope was systematically conservative relative to actual session velocity: each session shipped past its predecessor's "this is the next-session lead" framing within hours, often closing items the prior handoff explicitly recommended deferring (S202 close prose: "Pre-execution audit estimated ~30 min for EB diagnostic with P0-shaped deferral recommendation; both post-handoff items completed within time estimates"). Forward-audit horizons should bias towards concrete-shippable-this-session over deferred-for-context-budget; pacing models should track recent diagnostic velocity rather than worst-case context-budget projections. Candidate discipline: when writing close-session handoffs, frame next-session leads in terms of what one work-day can plausibly accomplish (e.g., "Track 1: ship X (1-2h); Track 2: ship Y (2-3h); Track 3 optional: start Z (60-120 min)") rather than "next session should defer Z, focus on X only" — the conservative framing systematically under-uses available context-budget. The codification rationale is that a stale pacing model causes the handoff to recommend pre-deferred scope that the next session then has to re-plan from scratch, which itself burns context. Promoted to candidate after S203 forward-audit identified five consecutive sessions of pace-exceeds-recommendation as the third-instance threshold (S198: planned-meta-only, shipped backfill design; S199: planned-windowing-only, also shipped SHADOW_ENTRY auto-heal; S200: planned-cohort-redefine-only, also shipped Protocol 7 framings-vs-hypotheses extension; S201: planned-converge-only, also shipped block-4 split + backfill draft; S202: planned-defer-EB-diagnostic, shipped both diagnostic and routing audit prep). Promote to numbered protocol if a sixth confirming instance arrives in the next 2-3 sessions; otherwise downgrade to memory-only as a planning-style preference rather than a binding rule.
