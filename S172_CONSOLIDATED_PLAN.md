...
- **Review-rebuttal-action chain as filter mechanism (codified S211, 2026-05-04).** Critical reviews and agent rebuttals operate as a filter mechanism on session-state findings. Reviewers should produce comprehensive critiques expecting some to be filtered; agents should rebut with primary-evidence references rather than accepting reflexively. Both roles ship signal-and-noise; the exchange is the filter that converts signal into action and noise into stale-snapshot rebuttals. **Application shape:** (a) reviewer surfaces critiques without overweighting individual confidence — comprehensive coverage is the value, not per-claim correctness; (b) agent triple-blinds each claim against primary evidence (commit history, files on disk, VPS state, prior plan text), categorizing as fully-correct / partially-correct / wrong; (c) agent's response cites primary evidence inline at each verdict, never paraphrasing without re-citation (Protocol 11 discipline carries through); (d) reviewer's pass-2 (if needed) accepts errors and contributes refinements; (e) operator decision shape is informed by the calibrated outcome, not by either role's first-pass framing. **Out of scope:** non-review feedback loops (e.g., operator-driven scope changes, mid-session pivots) — those flow through normal conversation discipline; this pattern applies specifically to formal review→rebuttal cycles where the reviewer is producing a structured critique against a session's close artifacts. **Pattern precedents (3 instances):** (i) S208→S209 chain — reviewer surfaced 11 critiques, 4 stale-snapshot + 7 valid; agent accepted 7 valid, rebutted 4 stale; net 4/11 false-positive rate (~36% noise filtered to 0% noise reaching action). (ii) S210→S210-review chain — reviewer surfaced 8 critiques, 4 valid + 2 partial + 2 stale; agent's rebuttal produced calibrated outcome including 6th-wiring-gap discovery; net 2/8 stale-snapshot rate (~25% noise). (iii) S211→S211-rebuttal chain (this codification) — reviewer surfaced 11 critiques, 3 fully-correct + 5 partial + 3 wrong; agent's triple-blind verified each against primary evidence; reviewer pass-2 conceded all 3 errors and contributed 7 refinements; net the exchange shipped Bundle A (5 plan-state items) + Bundle B (this codification + Protocol 15 + sister candidate). Promoted from §Protocol candidates per the S210 bucket-concentration / Protocol 14 precedent (promotion at 2 instances; S211 had 3 instances at promotion-time). **Companion candidate.** Sister candidate "Verify primary evidence before structural inference" filed S211 in §Protocol candidates — addresses the reviewer-side discipline that makes the rebuttal exchange high-signal. The two are complementary disciplines: this bullet codifies the agent-side rebuttal expectation; the candidate codifies the reviewer-side primary-evidence-first expectation. Bundle when the candidate promotes.
...
**7. Audit script extension — `getattr(settings, ...)` follow-through (S208, deferred). FULLY RESOLVED 2026-05-03 (S210, code commits `4142cbd` + `33abc0d`).** Current audit only catches direct `os.getenv("KEY")` and `os.environ.get("KEY")` calls. Bots commonly read via `getattr(settings, "KEY", default)` — same env var ultimately, but the read indirection was invisible to the AST walker. **Action (S210 code commit `4142cbd`):** added `extract_getattr_settings_calls()` AST pattern matcher to [scripts/config_drift_audit.py](scripts/config_drift_audit.py) (+58 lines, pure additive — existing `extract_getenv_calls` untouched). Pattern matches `getattr(ast.Name("settings"), ast.Constant(str), [default])`; intentionally narrow to first-arg = `settings` to avoid false positives from `getattr(self, ...)` etc. Both extractors yield into the same refs collection in `collect_code_defaults`, so categorize logic unchanged. New unit test file `tests/unit/test_config_drift_audit.py` (14 tests, all pass) covering both extractors against synthetic snippets. **Action (S210 code commit `33abc0d`, follow-up):** read+parse-once refactor + REPO_ROOT env-var override. The "VPS hang" turned out to be NOT doubled file I/O as initially hypothesized — the actual root cause was that `REPO_ROOT = Path(__file__).resolve().parent.parent` resolved to `/` when the script ran from `/tmp/`, causing `rglob("*.py")` to walk the entire VPS filesystem (including `/snap/core22/usr/share/gcc/python/libstdcxx/...` which surfaced misleading SyntaxWarnings). New `CONFIG_DRIFT_REPO_ROOT` env var lets the script run from any path with an explicit repo target. The read+parse-once refactor (new `_extract_*_from_tree` helpers + thin filepath wrappers for backward compat) is a separate optimization that halves the I/O+parse cost regardless. **Production verification (S210 same-session, post-`33abc0d`):** ran on VPS via `CONFIG_DRIFT_REPO_ROOT=/opt/polymarket-ai-v2 python /tmp/cda.py --no-ssh --local-env-file /opt/pa2-shared/.env` — completed in <30s. Production results: 878 unique code-referenced keys (vs 814 pre-extension baseline; +64 recovered, matches local), 117 keys in .env, **ENV-ORPHANS: 14** (vs S208 baseline 18 → **4 keys recovered on production** via getattr-settings extension). DRIFT count: 50 (vs S208 baseline 57). **Out-of-scope for this fix (filed as separate backlog):** direct-attribute access pattern `settings.KEY` (without getattr) — agent reports ~21 places across 5 files; same false-positive class but distinct AST shape; awaits separate code commit when priority warrants. File as §S210 Hygiene Backlog item if priority warrants, otherwise defer.
...
**2. VPS audit hang investigation (carries from §S208 Hygiene #7 qualifier). RESOLVED 2026-05-03 (S210 same-session, code commit `33abc0d`).** Hypothesis was doubled file I/O; actual root cause was `REPO_ROOT = Path(__file__).resolve().parent.parent` resolving to `/` when the script ran from `/tmp/` (filesystem-wide `rglob("*.py")` including `/snap/core22/...`). Two-part fix shipped in `33abc0d`: (a) `CONFIG_DRIFT_REPO_ROOT` env-var override so the script can run from any path with explicit repo target; (b) read+parse-once refactor (new `_extract_*_from_tree` helpers + thin backward-compat wrappers) — separate optimization that halves I/O+parse cost regardless. Production VPS run completed in <30s; ENV-ORPHANS dropped to 14 (vs S208 baseline 18 → 4 keys recovered on production). §S208 Hygiene #7 transitions from "RESOLVED LOCALLY" → "FULLY RESOLVED" — see §S208 Hygiene #7 above.
...
### Protocol 15 — Verdict-driving tooling verification

**Mandate.** Tooling whose output is treated as authoritative for any persistent operational state must have its correctness verified BEFORE the output is used as the basis for any operational decision. "Persistent operational state" includes (but is not limited to): canonical accounting ledgers, gate-evaluation verdicts, audit baselines, deploy/rollback triggers, sizing parameters, and any other output that gets cited or persisted across sessions. The discriminator question is: "does this script's output get persisted or cited later?" — if yes, the script is verdict-driving; if no (one-shot diagnostic), Protocol 15 does not apply.

**Verification minimum (any ONE of):** (a) test coverage on the verdict-producing function with a sample input/output sanity check, (b) code review by a second pass before the verdict is treated as authoritative, OR (c) cross-environment verification (run locally + on production, compare outputs) for tooling whose output is environment-sensitive. The verification bar matches the surface the tooling is gating: tooling that drives plan deviations or operational flips deserves the highest verification bar — same as the surface it's gating.

**Why this is a separate protocol, not a Protocol 6 amendment.** Protocol 6 governs canonical-source citation discipline ("cite `bot_pnl.py`"); Protocol 15 governs whether the canonical source is itself correct. Distinct enforcement layer. Protocol 6 is the citation discipline; Protocol 15 is the source-correctness discipline. Both are required: a correctly-cited wrong number is still wrong; a verified-correct number cited from the wrong source still violates Protocol 6.

**Out of scope.** One-shot diagnostic SQL or one-shot investigation scripts whose output is read once and discarded — those don't drive persistent state. Untracked `scripts/s{NNN}_invest_*.py` files (per §Operational Procedures session-local-investigation-scripts bullet) are exempt by definition, since their findings get codified in handoffs and the script itself is disposable. Protocol 15 applies to tracked tooling whose output is persisted, cited, or used as decision input.

**Evidence of origin (2 documented instances).**
1. **S195 (2026-04-25 → 2026-04-26) — RESOLUTION silent-zero pathology (17 days).** Production tooling at `insert_trade_event` RESOLUTION INSERT had a `--` SQL line comment that swallowed a tail clause; produced silent-zero RESOLUTION events; drove plan decisions about Bug A class for 17 days before discovery. Fixed in commit `b82ad68` via `/* ... */` block comment. This is the canonical-accounting-was-wrong shape.
2. **S207/S208/S209 (2026-05-01 → 2026-05-02) — Gate 5v2-C eval script Brier formula bug (~1 month).** `scripts/esports_v2_shadow_eval.py` computed Brier as `(p_model - 1{predicted_winner == actual_winner})²` but `p_model = P(team_a wins)` — outcome label inverted for predictions where p_model<0.5. Compounding bug: BSS climatology used `accuracy*(1-accuracy)` instead of `mean(y_a)*(1-mean(y_a))`. Both bugs pushed verdict toward FAIL/negative-BSS. Drove the §S208 PARK decision for ~1 month before discovery. Fixed in commit `5ecda0f` (S209). This is the gate-evaluation-was-wrong shape.

**Promoted from §Protocol candidates in S211 (2026-05-04)** at 2 evidence points per the S210 bucket-concentration / Protocol 14 precedent (also promoted at 2 instances). Per the S211 review-rebuttal exchange: scope wording was tightened from the original candidate's "operational decision (park, flip, deploy, revert, rollback, sizing change, etc.)" to the broader-but-discriminator-anchored "authoritative for any persistent operational state" — captures both evidence instances cleanly (S195 = canonical accounting; S207 = gate evaluation), excludes one-shot diagnostic SQL.

**Numbering note.** Protocol 15 takes the next available numeric slot after Protocol 14. Slots 8-10 remain reserved per Protocol 11's numbering note.
...
**Verdict-driving tooling verification — PROMOTED to Protocol 15 in S211 (2026-05-04).** Originally filed S210 (2026-05-03) per S210 review. Promoted in S211 at 2 evidence points (S195 SQL `--` silent-zero pathology + S207 Gate 5v2-C eval script Brier formula bug) per the S210 bucket-concentration / Protocol 14 precedent. Scope wording was tightened during the S211 review-rebuttal exchange from the original candidate's "operational decision (park, flip, deploy, revert, rollback, sizing change, etc.)" to the discriminator-anchored "authoritative for any persistent operational state" — captures both evidence instances cleanly (S195 = canonical accounting; S207 = gate evaluation), excludes one-shot diagnostic SQL. See `### Protocol 15 — Verdict-driving tooling verification` above for the full codified text. Pointer kept here for audit trail across cross-session references to "the verdict-driving-tooling candidate."
...
**Review-rebuttal-action chain as filter mechanism — PROMOTED to §Operational Procedures in S211 (2026-05-04).** Originally filed S210 (2026-05-03) per S210 review at 2 evidence instances (S208→S209 chain + S210→S210-review chain). S211 supplied a 3rd in-the-wild instance (S211→S211-rebuttal — the same conversation that promoted the pattern) before promotion. See §Operational Procedures bullet "Review-rebuttal-action chain as filter mechanism" for the full codified text. Sister candidate "Verify primary evidence before structural inference" filed in this section — codifies the reviewer-side discipline that makes the rebuttal exchange high-signal. Pointer kept here for audit trail across cross-session references to "the review-rebuttal-action chain candidate."

**Verify primary evidence before structural inference (sister candidate to Review-rebuttal-action chain bullet).** When making a "X may be caused by Y" or "X may be artifact of Z" structural-inference claim that depends on prior-session state, verify against the primary-evidence artifact (commit history, files on disk, captured output files) BEFORE asserting. The five-second header check (or grep, or `git log` invocation) is the prevention layer; structural inference shipped without primary-evidence check is the failure mode. **Why this is a separate candidate, not just a Protocol 7 amendment.** Protocol 7 governs hypothesis-inversion and framings-vs-hypotheses extension during execution; this candidate governs the prior-discipline of grounding structural inferences in primary evidence BEFORE the inference becomes an assertion. Distinct enforcement surface: Protocol 7 catches inversions during walk-backward-hypotheses; this catches them at the moment-of-assertion. **Evidence of origin (2 instances at filing).** (i) **S208 stale-snapshot review (2026-05-02).** Reviewer asserted critiques against draft state without verifying session was closed; produced 4 stale-snapshot critiques (1 commit chain ahead of reviewer's snapshot, 6 commits the reviewer hadn't seen) out of 14 total. Reviewer self-flagged post-rebuttal as a Protocol-candidate-shaped observation (filed as "Review-state-discipline" candidate at 1 instance). (ii) **S211 scope-artifact-without-verification (2026-05-04).** Reviewer asserted "S208 baseline of 18 ENV-ORPHAN may be artifact of REPO_ROOT-walks-/ bug" without checking the S208 audit output header (`S208_CONFIG_DRIFT_AUDIT_OUTPUT.txt:2` reads `Codebase: C:\lockes-picks\polymarket-ai-v2` — confirming local-Windows repo-scoped run). Agent's triple-blind verified against primary evidence; reviewer pass-2 conceded the error. Reviewer self-flagged as "structural inference shipped without primary-evidence check — same shape as S208 stale-snapshot." Both failures had the same structural shape: confident structural inference shipped without the five-second primary-evidence check that would have falsified it. **Why companion to Review-rebuttal-action chain.** The rebuttal-exchange protocol works because both sides have asymmetric discipline: reviewers produce comprehensive critiques expecting filtering; agents verify against primary evidence before accepting or rebutting. If only the rebuttal side is codified, reviewers can keep producing high-noise critiques because the protocol expects the agent to filter them — that's structurally fine for short-term but degrades long-term (high-noise reviews waste agent verification time even when correctly filtered). The sister discipline keeps the reviewer side honest. **Candidate disposition.** 2-instance file at filing. Promote at 3rd genuine occurrence per the S210 bucket-concentration / Protocol 14 precedent — but the discriminator question to ask at promotion-time: "is this Protocol-shaped (catches a recurring failure mode), or just a discipline reminder (codifies 'be careful')?" Per the S211 reviewer's own observation about the Verify-before-mark-RESOLVED candidate, codifying "be careful in general" doesn't help. Filed S211 (2026-05-04) per S211 review-rebuttal exchange.
