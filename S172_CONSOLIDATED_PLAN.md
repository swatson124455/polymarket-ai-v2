...
**SIZE_INVARIANT diagnostic — bot_pnl.py canonical P&L IS contaminated.** Day 3 review queued a focused diagnostic: pick 5 violations, manually compute share- vs dollar-denominated values from trade_events, cross-reference bot_pnl.py to determine which unit it reads. Result 2026-04-26:

- Sampled markets show two distinct inflation ratios — 0x06f6... (404.727 → 810.81 = exactly 2.0×) and 0x0008... (57.795 → 93.75 = 1.62×). Other sampled markets had EXIT.size ≤ ENTRY.size and shouldn't have been flagged at all (suggests the audit check's threshold/aggregation may also have a residual edge case, but the 2.0× and 1.62× cases are unambiguous real anomalies).
- `bot_pnl.py:130` reads `trade_events.realized_pnl` directly for both EXIT and RESOLUTION P&L. It does NOT recompute from `size * price` — but `paper_trading.py:683` *originally computes* `realized_pnl = (price - avg_price) * size - fee - _prorated_entry_fee` at EXIT-emit time. So the inflation propagates 1:1 into `realized_pnl` at write time and bot_pnl.py reads the wrong value forever after.
- Magnitude estimate: ~130 SIZE_INVARIANT events/day × ~70 days = ~9,000 affected events. Per-event error scales with `(exit_price - entry_price) * inflation_factor * entry_size`. For the 0x0008... sample: -$0.94 over-stated loss on one event. Across 9K events with mixed ratios and price deltas, cumulative bot_pnl.py drift is plausibly in the hundreds-to-low-thousands-of-dollars range. Not a typo magnitude.
- Likely emission paths: the over-sized EXIT cannot come from the standard mirror_bot.py exit (`size=exit_size = pos["size"]` at line 1352, which would equal entry size for a single-ENTRY position). Two candidates worth tracing first: `position_manager.py:467` auto-close-on-expired-market (`_size = float(pos.size or 0)` reads from `positions` table — if that row was inflated by some other path, the EXIT event inherits it); and `mirror_bot.py:283-310` `_seed_daily_exposure` / restore-state paths that may sum trade_events incorrectly into in-memory state.
- **Hold on every analysis of MirrorBot performance from bot_pnl.py until the inflation source is identified and either fixed forward or quantified for offset.** This includes the Phase 7 MB elevation gate (P(edge>0) ≥ 0.30 on post-Day-2 trades, S195 §forward-audit) — that gate's "ground truth" is bot_pnl.py output, which is contaminated. 7B Phase B counterfactual retune (post-soak ~May 9-10) similarly depends on bot_pnl.py being trustworthy.
