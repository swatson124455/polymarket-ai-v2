...
**5. Protocol 7 row 2/8 reclassification (S207 audit-derived). RESOLVED-FRAMING 2026-05-02 (S208, this commit). Substantive audit DEFERRED — see §S208 Hygiene #9 below.** §S206 Hygiene #1 row 2/8 disposition framed S196 and S202 handoffs as "gitignored handoff which would be the canonical record" / "plausible-but-unverifiable from plan content alone — testimony lives in gitignored AGENT_HANDOFF_S202_CLOSE.md." S207 verified both `AGENT_HANDOFF_S196_CLOSE.md` and `AGENT_HANDOFF_S202_CLOSE.md` exist in working tree (gitignored from commit per `.gitignore:147` but readable). Reclassified Row 2/8 dispositions from "blocked / pending / unverifiable" → "audit deferred (handoff readable, audit not yet run)." Net audit count unchanged (12 inversions / 8 sessions); only Row 2/8 disposition framing corrected. Audit-derived sub-finding extended: future Protocol 7 row additions should cite "working-tree-only AGENT_HANDOFF_SXXX_CLOSE.md" rather than "gitignored AGENT_HANDOFF_SXXX_CLOSE.md" to avoid the same misframing. **Action (S208 commit):** all four phrase-instances of "gitignored" framing in §S206 Hygiene #1 RESOLVED note replaced with "audit deferred (readable in working tree)." **S208 close-review clarification:** RESOLVED here refers to the framing-correction action only. The substantive audit (actually reading both handoffs and verifying the inversion attributions for Row 2 SIZE_INVARIANT FIX_AUDIT_CHECK→FIX_EMISSION rescoping and Row 8 S202 plan-revision-approach inversion) is still pending. Filed as §S208 Hygiene #9 for next-session execution.
...
**Operator-decision-queue batching (§Operational Procedures candidate, NOT Protocol candidate).** Session-close handoffs surface operator-blocking decisions in §Operator actions required sections. When the queue is small (≤3 items), per-§5-item presentation is fine. When it grows (≥5 items observed in S208 close — EB v2 verdict, 2 modified working-tree files, 2 stale memory files, BOT_ENABLED_ESPORTS_SERIES removal, pre-send P11 codification, audit-corrections-bundling codification = 6 items at peak), decision fatigue becomes a real concern: per-item presentation invites incremental scope creep, whereas batch presentation invites batch resolution. Candidate discipline: when session-close operator queue is ≥5 items, surface as a top-of-handoff "Operator Decisions Pending" block with one-paragraph context per item and a single batched-decision invitation alongside the existing §5 enumeration. **Evidence-of-origin (1 instance):** S208 close handoff §5 listed 6 operator-blocking decisions; S209 plan-review surfaced the queue-accumulation as a structural concern. **Candidate disposition:** §Operational Procedures candidate (operational discipline, not a binding rule for all sessions). Codify after one session of observed-effective batched-decision-pass — a session where operator processes ≥5 queued decisions in batched form and the handoff gap closes. Until then, soft observation. Filed S209 (2026-05-02) per S209 plan-review.
