...
- **Secondary bug:** buying-power probe `clob_adapter.check_pusd_balance` reads `0xC011a7E1…82DFB` ($0.31782), but real collateral is USDC.e `0x2791…4174` (`Exchange.getCollateral()` + position-ID math confirm). `0xC011` is NOT the collateral. **Even after a UI redemption credits USDC.e, the bot may not see it as buying power until this probe is fixed.** Verify + code-fix before the closed loop works.
