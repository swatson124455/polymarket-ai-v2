...
- **Live endpoints work for sports too:** `/api/v1/{soccer,nba,tennis,mlb}/odds`
  all 200 with live events — a forward live recorder for sports is possible
  (NOT built; see §4).
...
1. **Live sports recorder** (forward collection, EB-recorder pattern, would need
   a cron line in the SHARED crontab — WB/EB lines live there, so operator
   sign-off before touching it). Valuable because `closing-odds` is broken:
   closing lines must come from our own snapshots or per-event history.
