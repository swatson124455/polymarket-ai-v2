...
>
> Operator answers to the seven decision points (verbatim: "1 rec 2 force
> safely 3 rec 4 rec 5 lets remove tiers bet full, this NEEDS TO BE CHANGED
> BEFORE LIVE THOUGH 6 no moves needed 7 rec"):
> 1. **PR #15 merged** → master 2b82775d (P1–P5 + D1 fix + docs).
> 2. **Clone force-refreshed AS polymarket** (never root) after 7; verify
>    line = clone HEAD vs origin/master + md5 of the six changed scripts vs
>    master BLOBS (recorded below when done).
> 3. **Tripwire re-ruled — DROPPED is lock-only** (futility / DNQ); the
>    "LCB<0 at n≥10" clause retired (it fired on 0x4ab40f2a49: roi +1.128,
>    lcb −0.781 @ n=54). DEGRADED (realized mean <0 at n≥5) kept. PR #16.
> 4. **Roster +2, watch-only:** 0x75a27d0cc9 + 0xe9f5c75ee1 as group
>    `cohort8` in chain_audit.json (clean 127 → 129; backup
>    `.pre-cohort8-20260908`), watcher `systemctl restart`; the grader
>    registers them automatically on their own clock at the first run of the
>    merged code. $0 everywhere; real money still docs/MB_GO_CHECKLIST.md.
> 5. **Allocator tiers OFF** — `MB_ALLOC_TIER_FRACS=` blank in
>    /opt/pa2-shared/mb_sizer.env (backup `.pre-tiersoff-20260908`): funnel
>    display stakes use the FULL bankroll per trader (boards/ledger never
>    used the envelopes, so the $algo headline does not move — that headline
>    is the concurrency divisor + min_viable, the sizer's own D3/E5 design;
>    say the word if THAT is what "bet full" meant). **GO CHECKLIST ITEM 6
>    ADDED: tiers must be re-ruled before live.** Rollback: restore the
>    backup file.
...
> ## `claude/mb-tailable-0908`; D1 FIXED; LIST GENERATED BY HAND; NOTHING
> ## DEPLOYED YET — DEPLOY = MERGE TO MASTER + ONE POST-MERGE SCRIPT
>
> **What exists now (all on the branch, PR to master pending operator):**
> the verified-tailable LIST generator, the forward MONITOR (grader on
> per-group clocks + machine-readable forward status + tripwire label),
> the CANDIDATE PIPELINE (list → dive queue → serial runner → dossiers
> back on the list), ROI at OUR SIZER stake on both boards / funnel /
> ledger ($algo first, $100 ref = comparison), and FILL REALISM on the
> firehose board. Zero real orders; every $ HYPOTHETICAL; real money only
> via docs/MB_GO_CHECKLIST.md (unchanged).
> **Commits (in order):** 67e20692 D1 `effective_epoch` · 5a602bb2 roster
> registration · 8282fb4f forward-status artifact + tripwire · 413ce156
> funnel/ledger enumerate roster groups · 4f66ec27 list generator + null
> seed · 5ffeb1a5 cron stage 11 + chain-watch 11 · bc543a1d, 7fa43c60 fixes
> · 440e5c31 P3 boards $algo · 891b162b P3 funnel/ledger · 87c0672d key fix
> · 445b803f P4 pipeline + runner + stage 12 · 68f9ad8f P5 fill realism ·
> 7fd8f447 −0.00 · afba2260 `deploy/mb_tailable_post_merge.sh`. Every
> commit: self-test + mutants (all killed) + dependents' tests; unit tests
> tests/unit/test_mb_backtest.py 15/15, test_mb_sizer 36/36 incl.
> **D1 FIXED (67e20692, operator-approved via ruling 1):** the grader
> scores each group from `effective_epoch(group) = max(group admission,
> BASIS_EPOCH)`. Every pre-conversion group is unchanged (still
> 2026-09-06T22:30Z); a post-conversion roster group runs its own clock.
> Cohort5's futility date moves 2026-09-13T22:30Z → 2026-09-15T02:56:10Z
> (from chain_audit.json `cohort5.admitted_utc`). Registration is now
> AUTOMATIC (5a602bb2): dive ADMIT (newest dossier across 7 dirs, mtime
> wins) + roster (`clean`) + group `admitted_utc ≥ BASIS_EPOCH` + not in a
> hand list; every exclusion printed. Known-data replay against the real
> roster/dossiers 19:09Z: exactly cohort5's 3 wallets registered, epoch
> 02:56:10Z; 122/127 roster wallets are in hand lists; 2 in no group
> (0x448861155279 cohort4, 0xfbfd14dd4bb6 probe — pre-conversion, OBS,
> unchanged). Locks file untouched (7 keys; none of the 3).
> **MONITOR:** the grader writes `deep_dive/cohort5_forward_status.json`
> (per address: epoch, el_days, futility_utc, n, wagers, e, roi, lcb,
> wk_ref100_lcb, verdict, tripwire) at both exits; funnel shows
> roster-admitted wallets as TRIAL `roster-cohort5` on their clock; ledger
> accrues them from admission. Tripwire (label only, never a removal):
> lock verdicts first, then DROPPED (lcb<0 @ n≥10), DEGRADED (roi<0 @
> n≥5), else WATCH — the plan's numbers as written.
> **⚠ FLAGGED FOR RULING (implemented as written):** `mc.roi_lcb` is
> sup{m: e(m)≥20}; "LCB<0" = not-yet-shown-positive, not negative. On the
> 17:58Z firehose board 0x4ab40f2a49 has roi +1.128 with lcb −0.781 at
> n=54 — the plan's DROPPED rule fires on a strongly positive trader.
> Operator: keep, or re-rule the DROPPED test (e.g. futility/lock only).
> **THE LIST — hand-run 19:25:16Z as polymarket (boards 17:58Z firehose /
> 17:20Z roster, labels 704,350 keys @ 17:31:47Z, HYPOTHETICAL $100/wager
> LCB/wk): VERIFIED 5 / PENDING 6 / FLAGGED 0 / DROPPED 0 / EXCLUDED 0,
> universe 11.** VERIFIED: 0x5feea3460c +$14,659 (n_ho 134, conc 18/18,
> cov 94, selection Y p=0.00, cohort5); 0xe48217d0b7 +$1,312 (p=0.19,
> cohort5); 0x2c50852938 +$789 (p=0.47, cohort5); 0x75a27d0cc9 +$90 (dive
> ADMIT 18:54Z mkts 73 P=0.9975; NOT rostered); 0xe9f5c75ee1 +$22 (ADMIT
> 19:17Z mkts 1038 P=1.0; NOT rostered). PENDING: 0x4ab40f2a49 +$6,704
> (INSUFF span 48d); 0x1c010e69db +$1,265 (INSUFF span 58d → pipeline
> re-dive on/after 2026-09-10); 0x5cb1327663 (elig FAIL, first trade
> 2026-09-05T21:10:43Z); 0xf7d03961dd +$91 (INSUFF P=0.19); 0x4ed3513040
> +$90 (no dossier → pipeline NEW dive); 0x5d2e0736fb −$71 (INSUFF). All
> 11 eligibility reads done (data-api ASC, shape re-proven against
> 0x2c50852938's known first trade 2026-05-24T14:01:35Z). Dive queue
> COMPLETE 19:26:55Z rc=0 (10 dossiers): 0x4ab40f2a49 INSUFF, 0xf7d03961dd
> INSUFF, 0x75a27d0cc9 ADMIT, 0xe9f5c75ee1 ADMIT, 0x4f453abb65 INSUFF
> (mkts 61, span 148d, P=0.43).
> **$ALGO LIST (the ruled basis) — hand-run boards 20:21Z on the P3+P5
> code + list 20:22:28Z, same label vintage, firehose rows FILL-MODELED
> (x0.81–0.89), snapshot docs/MB_TAILABLE_LIST.md; HYPOTHETICAL $/wk LCB
> at OUR sizer stake, foursome read from mb_sizer.env = bankroll 500,
> kelly_mult 0.25, concurrency floor 1, min_viable 1:** VERIFIED 5 /
> PENDING 7 / universe 12. VERIFIED by $algo: 0x5feea3460c **+$222**
> (vs $ref100 +$13,351; stake_med $1.70; n_ho 145); 0xe48217d0b7 +$8
> ($ref100 +$485); 0x75a27d0cc9 +$1 (+$72); 0x2c50852938 **+$0** (+$688;
> stake_med $0.00); 0xe9f5c75ee1 +$0 (+$87). PENDING: NEW board #1 by
> $algo 0xd6730ad2d5 +$2,625 (conc 2 → $62.50/slot; stake_med $34.44;
> $ref100 +$431) but eligibility FAIL (first trade 1.7d ago); 0x5cb1327663
> +$221 (FAIL, 3.0d); 0x4ab40f2a49 +$147 ($ref100 +$5,847, INSUFF);
> 0x4f453abb65 +$15; 0x1c010e69db +$0 (lcb +0.007 now); 0x5d2e0736fb,
> 0xf7d03961dd −$0 (LCB<0). **FINDING (report, not a change):** at this
> foursome the sizer's per-slot budget is bankroll×kelly_mult/peak_conc
> (= $6.94 for conc 18) times a Kelly fraction from LCB×fill — cheap-fill
> laddering wallets land under min_viable $1 and clamp to $0 (never up),
> so the ruled headline says OUR ALGO WOULD BET ALMOST NOTHING on the top
> $100-reference wallets today; the ranking by $algo puts low-concurrency
> wallets first. The foursome is the operator's; nothing was tuned.
> **ROSTER PROPOSALS (operator ruling; present, not push):** 0x75a27d0cc9
> and 0xe9f5c75ee1 are VERIFIED on all five criteria but tiny at the
> $100 ref and their board replay verdict is NOT DEMONSTRATED (futility).
> **P5 MEASURED (19:40Z, shadow sink):** 67.9% of 171,012 BUY records
> clear the live gates (first-buys 46.8% — the audit's 47%); 0.9–1.0
> price bucket 31%, others 75–89%. Firehose $ columns are now weighted by
> each wager's bucket rate (raw kept beside); roster rows keep real gates.
> **DEPLOY (operator):** merge the PR → the clone picks it up at the next
> 12:30Z refresh (or force one AS polymarket) → `sudo bash
> /opt/pa2-shared/mb_readout/deploy/mb_tailable_post_merge.sh` (adds
> stages 11+12 to /opt/pa2-shared/mb_chain_stages.json, points the
> stall-watch 'dive' job at the shared fill cache, installs the 13:00Z
> runner cron; idempotent; `--check` first). Do NOT run it before the
> code is in the clone. Verify deployed md5s against master BLOBS.
> **WATCH:** 09-09 11:40Z = OLD code: [chain] 10/10 no INTEGRITY note +
> a "gamma window crawl" section; 09-09 12:30Z: no "refresh FAILED". First
> run on the new code (12 stages) = the 11:40Z after the refresh that
> carries the merge: grader block shows "roster-admit cohort5 (3)" with
> three ACCRUING lines and writes cohort5_forward_status.json; list +
> pipeline stages print; [chain] 12/12.
> **LANDMINE HIT THIS SESSION (again):** `pgrep -f <pattern> | xargs kill`
> in an ssh one-liner matched the remote shell's OWN cmdline and killed
> the session (exit 255). PID exclusion is mandatory even for one-liners.
> Also: a bash heredoc through the tool collapsed one backslash level and
> shipped a cron line with literal `\n` (bash -n cannot catch it) — fixed
> in 7fa43c60; write shell continuations with the Edit/Write tools.
