...
| 07-21 | **REWARDS RECEIPT CONFIRMED (operator):** balance $55.35→$74.70 rise since 07-21 handoff = **LIP maker rewards credited** (~$18.6), NOT settlements (settlements = ~$0.72 dust, market_result rows revenue 0/12/29/31¢). First RECEIPT-grade evidence rewards actually pay out to this account. No API ledger endpoint (all 404) — operator confirmed via Kalshi web UI. | rewards are real money in | operator confirm + settlements/fills probe |
| 07-22 pm | **THROTTLE A/B RESOLVED — keep 1-tick step (arm A).** Live A/B was SOFT and is disowned: 3 variables changed mid-test (naked-risk fix deployed, +$20 deposit, cap 65→85), arm-A baseline n=1, and it never measured rewards at all (arm B filled so constantly it produced no quiet interval, the only window the residual method reads). Replaced with a SANDBOX paired study (`kalshi_live/kalshi_ab_throttle_study.py`, read-only/no-keys, imports the CFTC scoring core from `scripts/maker_kalshi_recorder.py`): both arms scored on IDENTICAL book snapshots. **n=612 market-snapshots: at-reference = 1.59× the reward of 1-tick-inside (share .168 vs .106); the tick step ZEROES credit entirely in 12% of snapshots** (falls out of the qualifying set). Cost side (not simulatable — queue position) measured LIVE: arm B ~tripled naked-inventory build ($11→$30 in ~40min, 2 ceiling trips). **Verdict: forfeit ~37% reward to cut exposure build ~2/3 — correct at this size** (one bad accumulation = the morning's $21 > a day of rewards). FUTURE LEVER (not done): step back ONLY while actually throttling, stay at reference when flat — recovers most of the 1.59× ; testable in the same sandbox for free. | `THROTTLE_STEP_TICKS=1` deployed | study n=612; commits `b9a1c7e` |
| 07-22 pm | **BREAKERS NOW MEASURE NAKED RISK, NOT GROSS** (`naked_held_cost()`): live, $27.76 held of which $11.84 was a FLOORED ladder pair — gross-gating pinned the bot REDUCE-ONLY over risk it did not carry, blocking ALL earning. After fix: breaker cleared, quoted 2→5 markets, committed $30→$60. **CAPITAL cap deliberately still uses GROSS** (paired inventory really does consume cash). Also: **⚠ LADDER SELF-HEDGE WAS DEPLOYED UNREVIEWED** — `git show HEAD` carried it while commit messages said "NOT deployed"; the 2 known bugs were already fixed pre-deploy and live behaviour verified correct (pairing exactly explains resting unwinds), but the blind review never completed (46/68 agents died on spend limit). **OWED: finish the ladder review.** | risk gates fixed; review debt open | `d40665e`,`fbd5abb` |
| 07-22 pm | **CAPITAL $85 / DAILY QUOTA $40** after operator +$20 deposit (equity $63.89). **⚠ DEPOSITS CORRUPT THE LOSS METER** — it reads equity, so a deposit registers as profit and silently loosens the brake (would have hidden ~$60 of real loss behind a $40 quota). Manually re-baselined; **any future deposit/withdrawal needs a re-baseline**. Loss meter armed at $63.89. | knobs raised, meter honest | live.env |
| 07-22 | **DEPLOYED + LIVE (operator "proceed", ~02:26 UTC).** Sequence: `round→int` truncation fix (`aec075f`, 52/52 tests) → LF artifacts md5-GATED install (quoter `610dca7e`, client `3599d513`; backups `.bak-*`) → env `INV_TOLERANCE=3→1` (deadband = venue min order; invisible ≡ un-actionable) → import check w/ env (clamp fired: SETTLE 30→20) → STOP removed → **hand-run cycle CLEAN** (`cycle ok` fp=40 quoted=8 creates=16 fails=0 taker=0 committed $89.83/90 reads=41) → first REAL fill within ~1min (KXTEMPDCH +11 ct, <SOFT → maker unwind next cycle; CHI −1 ct actionable under TOL=1) → timer enabled 2-min. Watch: taker_flattens=0, 429s, unwind behavior, per-event delta. Balance $67.44 w/ collateral resting. | LIVE, delta-neutral build w/ all review fixes | this session; journal 02:26-27Z |
| 07-21 | **FIXES SHIPPED TO BRANCH (`262819f` + `2ec1d99`) — bot still STOPPED, NOT deployed.** All 4 HIGH fixed (C4 cursor-pagination + count_filter, probe-verified params incl. `limit=1000`→HTTP 200; C1 IOC partial-fill no longer raised as rejection; C2/C3/C15 blackout guard: streak clears only after BOTH reads good, last_oids includes this cycle's venue ids, failed cancels kept for retry) + config/economics (C8 settle-taker clamped ≤ wind-down; C13 per-market lifetime-relative ramp; C18 footprint round-robin so gas isn't starved) + MED safety (C12 per-market held-$ cap, C7 no unwind-stacking, C6/C10 2× unwind drain, C5 nonced STOP ids, C16 settle-check telemetry, C17 flock run lock, reconcile_fail→blackout streak). **New quoter md5 (LF) `f3a0df90`, client `3599d513`** (pre-fix was `10d92941`/`e0b4c9c0`). Suite **51/51**; **16 of them FAIL on pre-fix `ea28fa38`** (measured by running the new suite against the old build) — 1 test is a deliberate regression guard, not a pin. Smoke clean, dry_run tripwires 0. **BLIND adversarial review of the fix diff: ZERO confirmed regressions / new bugs** (5 actionable items, all refuted or LOW/INFO; 4 verifier agents died on a spend limit → their findings re-derived from the journal and closed by hand). Residuals still OPEN by design: C9 (conflicting inv/event-sign unwind grows \|event\|), C11 (single-cycle multi-strike leak), C14 (cross-event correlation), SPLIT (unwind can be balance-rejected; no balance-aware sizing — C6/C10 widening slightly enlarges this surface). | ready for operator go/no-go on deploy | commits `262819f`,`2ec1d99`; wf 15-agent blind review |
...
| 07-23 02:05Z | **SESSION-OPEN VERIFICATION of handoff §0 — all state checks PASS, two handoff claims CORRECTED.** Verified read-only: deployed `/opt/pa2-maker-kalshi-live/maker_kalshi_quoter.py` md5 `727ca7c59840a42b51c19e24c65a0982` == branch-HEAD git blob (byte-identical); `polymarket-maker-kalshi-live.timer` active 2-min + `…-ledger.timer` hourly + `kalshi-plugin-off.timer` armed; **no STOP sentinel**; suite **83 passed** + `dryrun_smoke.py` clean (forbidden-call tripwires all 0); account balance $40.85 + held $24.13 = **equity $64.98** / 8 positions / 5 resting (vs handoff $64.74 — flat, no loss event); **`ladder_violation` absent from every plan row = 0** (note `ladder_cross` is the hedge-ACTION counter at `maker_kalshi_quoter.py:1089`, NOT a violation — do not read it as one). Per-event delta: KXAAAGASD-26JUL23 −0.18 ct (~flat), KXAAAGASW-26JUL27 −27 ct (>TOL). **CORRECTION 1 — handoff §1/§5 "the box predates the failed-taker fix" is STALE:** deployed md5 == HEAD and HEAD *contains* the fix (`maker_kalshi_quoter.py:941-957`: only a CONFIRMED flat retires the ticker; a failure increments `taker_failed` and falls through to maker unwind). Owed item #1 therefore needs **NO deploy** — it is a one-line `live.env` flip `KALSHI_TAKER_FLATTEN=0→1`. **NOT flipped: deliberately deferred past the A/B window**, because moving a knob mid-test is exactly what disowned the 07-22 live throttle A/B. **CORRECTION 2 — the A/B had NOT flipped** (see next row). | §0 trustworthy; 2 stale claims retired | this session; VPS read-only probes + `git show HEAD:… \| md5sum` |
| 07-23 02:05Z | **A/B (reduce-only two-sided plug-in) IS STILL RUNNING — NOT a completed result.** Box clock 02:04Z; `kalshi-plugin-off.timer` fires **04:38:22Z** (2h34m out). The OFF arm is sampled AFTER the flip, so it has **zero cycles** — there is no paired comparison yet, only one arm. `kalshi_ab_plugin_report.py`: **ON arm cycles=13, reduce-only 76.9% of them, 62 market-cycles, TWO-SIDED 79.0% overall / 77.3% during reduce-only** (the 44 market-cycles the plug-in actually changes); mean naked $22.12, mean committed $64.46, ladder_violations=0. Directionally strong vs the pre-plug-in live observation of 3/3 markets one-sided, but that baseline was eyeballed, **not produced by this tool — not a paired number**. ⚠ **Variance caveat:** the two most recent cycles ran 1 two-sided / 3 one-sided (**25%**), far below the 77.3% mean — n=13 cycles is small. **NOT MEASURED (tool cannot):** rewards per arm — reduce-only is precisely when the bot is filling, so the clean quiet intervals the rewards method requires do not exist there. TWO-SIDED % is the honest proxy: it is exactly what the CFTC rule gates. **Re-run after 04:38:22Z for the real two-arm readout.** | one arm only; do NOT report as a finished A/B | `kalshi_ab_plugin_report.py` on box; `ab_plugin_marker.json` = `{"arm_on_start":"2026-07-23T01:38:21Z"}` |
| 07-23 02:05Z | **MIRROR-SYMMETRY TESTS RESCUED + COMMITTED (`244c24f`), AND THEIR DOCUMENTED STRENGTH IS OVERSTATED — measured, not assumed.** (a) The 7 tests handoff §4 lists as an in-force guardrail were **never committed** — they sat as an uncommitted working-tree diff in the PRIOR session's scratchpad worktree, one cleanup away from being lost. Rescued by `git worktree move` (not delete) and committed test-only, no runtime change. Pin measurement: **HEAD alone = 76 tests, HEAD+these = 83**, which is the count the handoff documents — i.e. every "83 tests" claim on record was really 76 committed + 7 uncommitted. (b) **MUTATION TEST — the 7 tests pin 0 of 3 polarity bugs.** Injected three independent live sign-flips (`:511` reducing-side selection `inv > 0`→`inv < 0`; `:554` throttle direction `acc` sign swap; `:1080` ladder hedge price side swap). Full suite caught all three (**1 / 8 / 1** failures respectively, so all three are genuine behaviour changes, not dead code) — but the **mirror-symmetry subset caught ZERO (7 passed on every mutant)**. **Root cause, and it is structural:** mirror symmetry is INVARIANT under a *consistently applied* sign flip — flip polarity everywhere and both the long-YES case and its mirrored long-NO case flip together, so the property still holds. **Therefore handoff §4's claim that these tests catch "a polarity bug [that] would throttle the wrong side or 'unwind' the wrong way" is WRONG as written.** What they actually guard is the narrower class of *asymmetric* bugs — logic that treats YES and NO differently. Real coverage against global polarity comes from the other 76 tests. Keep the tests (the asymmetry class is real and worth pinning); **fix the claim, do not trust it.** Worktree restored to pristine after mutation runs; md5 re-verified `727ca7c5` == deployed, so no live artifact was ever at risk. | guardrail is real but NARROWER than documented | commit `244c24f`; mutation runs this session |
