...
## AMENDMENT 2026-09-01 (operator-adopted, item D6) — SELECTION IS NEVER OPEN-LOOP PAST THE FIRST RECEIPT

The Kalshi lane's terminal failure (their 09-01 selection review, central
finding) was a live picker that ran on pool size and an uncalibrated model for
its ENTIRE run, with zero receipts/accrual feedback. Binding here: from the
FIRST live receipt onward, the receipt-vs-model calibration table (§0b) feeds
market selection — a market class whose receipts undershoot its model loses
model weight at the next selection pass, measured not assumed. Live selection
never runs more than one receipt cycle on an uncalibrated model. Wiring is
post-first-receipt work; this amendment is the commitment.
