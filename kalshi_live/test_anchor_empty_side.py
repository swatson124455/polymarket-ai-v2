...
def test_099_touch_book_is_structurally_unanchorable():
    """v3 (live-measured 17:08-17:11Z): on a 0.99-touch book the implied opposite ask is
    0.01 — a 1c anchor post-only REJECTS forever and the join leg rests ALONE (unpaired).
    v2's step-down produced exactly that. Correct behavior: refuse, one-sided gate."""
    quotes, stats = _run([["0.99", "500"]], [])
    assert quotes == [] and stats.get("gate_one_sided_book") == 1
    assert "anchor_paired" not in stats


def test_098_touch_book_anchors_at_touch():
    # implied opposite ask = 0.02 > anchor 0.01 -> legal: join AT 0.98, anchor 0.01
    quotes, stats = _run([["0.98", "500"]], [])
    assert stats.get("anchor_paired") == 1
    sides = {o["side"]: o["price_dollars"] for o in quotes}
    assert sides == {"yes": 0.98, "no": 0.01}


def test_mid_extreme_touch_anchors_at_touch():
