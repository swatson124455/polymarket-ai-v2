...
|---|---|
| `pattern.dedup_mechanisms`: "DISCARD the one-bet-per-market policy" | **KEEP.** One-bet-per-market is a binding CLAUDE.md constraint AND the neg-risk-specific safety mechanism that replaced the reverted blanket `neg_risk` filter (`f66ed43`). Re-implement all four guard scopes: `mirrored_trades` capped-OrderedDict (same-side dedup), `mirror_opposing_side_blocked` (cross-scan), `mirror_opposing_side_blocked_historical` (cross-session), `_entered_market_sides` restored at startup. |
...
- **Gate mechanics:** point-in-time fill-replay against `orderbook_snapshots` at signal timestamps from cleaned `mirror_rejected_signals` (4-step cleaning recipe, manifest §Tier-2), VWAP book-walk fills, no look-ahead; then `edge_verification.py`-style bootstrap: rule passes only if P(edge>0) clears the gate threshold on the replay P&L.
...
| M2 | M0-DB data verification | Script by session; **run by operator on VPS**. Verifies row counts/date ranges, the never-verified gate-stage labeled intersection, orderbook coverage at replay token_ids, `side='SELL'` corruption count. **All data-dependent work blocked until this passes.** |
...
| M5 | M-ENV reconciliation before first paper deploy | Operator env dump MUST include `CANARY_STAGE`, `CANARY_AUTO_ADVANCE`, `MIRROR_*`, `BOT_BANKROLL_CONFIG`, `POLYGON_RPC`, `RELAYER_API_KEY`. Per-knob record: inherit vs deliberate reset. Sizing resets to code defaults per Decision 4. |
