...
## Status (as of 2026-03-06 Session 55)
| Field | Value |
|-------|-------|
| Enabled | YES (BOT_ENABLED_ESPORTS=true, isolation mode) |
...
| VPS State | RUNNING — scanning, finding 0 tradeable markets |
| Last trade | 13 paper trades (pre-fix era, none resolved) |
| Blocker | Seasonal: no live esports markets on Polymarket as of early March 2026 |
...
5. Fetch esports markets from Polymarket (limit 200, category="esports")
...
| LoL model pkl | data/esports_lol_model.pkl (if trained) |
| CS2 model pkl | data/esports_cs2_model.pkl (DELETED — degenerate; will retrain) |
...
| Polymarket API | YES | Esports market discovery (category="esports") |
...
|----------|---------|-------------|---------|
| BOT_ENABLED_ESPORTS | false | true (isolation) | Enable gate |
...
| ESPORTS_MODEL_MAX_BRIER | 0.24 | 0.248 | Brier graduation threshold (no-skill=0.25) |
...
| RISK_MIN_PRICE_ESPORTSBOT | not set | not set | Per-bot price floor override |
| RISK_MIN_VOL_ESPORTSBOT | not set | not set | Per-bot volume gate override |

## Model Status
| Model | Accuracy | Brier | ECE | Graduated? | Notes |
|-------|----------|-------|-----|------------|-------|
| CS2 Economy (cs2_economy_model.py) | 58.2% | 0.2473 | 0.0953 | YES | Graduated Session 55; calibrated+saved |
| LoL Win (lol_win_model.py) | 51% | 0.25 | 0.0106 | NO | Heuristic fallback; needs live game data |
| CS2 heuristic (_predict_round_heuristic) | N/A | N/A | N/A | N/A | Fallback when CS2 model not loaded |
| Universal fallback (prediction_engine.py) | 53% | 0.2418 | N/A | Paper phase | Used when game model not graduated |

**Session 55 model changes:**
- Label leakage fixed: outcome-derived features (gold, towers, dragons for LoL; round scores for CS2)
  neutralized to prevent tautological training. Both in data collector and post-load sanitizer.
- XGBoost complexity reduced: depth 5→3, estimators 150→80 (CS2); depth 6→3, estimators 200→80 (LoL).
  With only 1-2 real features, deeper models overfit.
- Brier graduation threshold raised to 0.248 (env: ESPORTS_MODEL_MAX_BRIER=0.248).
  CS2 at 0.2473 genuinely beats no-skill baseline (0.25).

**Training data reality:**
- ALL economy features (money, equip_value, round_score) are constant placeholder defaults
- `round_score_a/b = 6.0` for all rows — PandaScore free tier has no round-level data
- `_get_team_strength()` always returns 0.5 (API returns 403) → Glicko-2 used instead
...
- **[Session 55 — FIXED]** CS2 model graduation: Label leakage neutralized, XGBoost depth 5→3,
  Brier 0.2507→0.2473. Threshold 0.248. Graduated+saved. Commit: 8472936.
...
- **[Session 53 — FIXED]** XGBoost deprecation warnings: Commit: dc154bf.
- **[OPEN]** LoL model 51%: Needs premium PandaScore or live game data for improvement.
- **[OPEN]** 0 tradeable markets: No live esports markets on Polymarket as of March 2026.
- **[OPEN]** Prediction logging silent failure: 0 rows in esports_prediction_log despite 13 trades.
...
# Live logs
ssh -i "$KEY" "$VPS" "sudo journalctl -u polymarket-ai -f | grep EsportsBot"
...
# Check recent esports paper trades
ssh -i "$KEY" "$VPS" "sudo -u postgres psql -d polymarket -c \"
...
# Check if any esports markets in DB
ssh -i "$KEY" "$VPS" "sudo -u postgres psql -d polymarket -c \"
  SELECT COUNT(*), active FROM markets
...
## Next Steps / Blockers
- [ ] Wait for live esports markets to appear on Polymarket (LoL Worlds / CS2 Major / Valorant Champions)
- [ ] When markets appear: validate confluence score thresholds with live data (start conservative, tune)
- [ ] Improve LoL model: need premium PandaScore API or live game data for round-level features
- [x] Graduate CS2 model (Session 55): Brier 0.2473, threshold 0.248, graduated+saved
- [ ] Fix prediction logging: investigate why esports_prediction_log has 0 rows despite 13 trades
- [ ] Consider upgrading PandaScore tier to unlock Glicko-2 endpoint + round-level data (key blocker)
- [ ] Re-enable EsportsLiveBot and EsportsSeriesBot after EsportsBot proves profitable with live data
