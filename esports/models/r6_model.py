"""
Rainbow Six Siege Win Model — XGBoost with 6 Glicko-2 features.

S136 Phase 11D: Logistic regression on Glicko-2 features.
Targets R6 Pro League / Blast R6 (~16 teams). Attack/defense
operator ban features added as data accumulates.
...
MODEL_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", "..", "saved_models", "r6_xgb.json"
)


class R6Model:
    """XGBoost binary classifier for Rainbow Six Siege match winner prediction."""
    ...
    def predict(self, game_state: Dict[str, Any]) -> float:
        ...
        try:
            ...
            logger.debug("R6Model: predict failed", error=str(exc))
            return 0.5

    def train(self, training_data: List[Dict[str, Any]]) -> bool:
        """Train on historical match data. Returns True if successful."""
        if len(training_data) < 30:
            logger.warning("R6Model: insufficient data", count=len(training_data))
            return False

        try:
            return self._train_sync(training_data)
        except Exception as exc:
            logger.error("R6Model: training failed", error=str(exc))
            return False
    ...
    def save(self, path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.info("R6Model: saved", path=path)
            return True
        except Exception as exc:
            logger.warning("R6Model: save failed", error=str(exc))
            return False
    ...
    def load(self, path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.info("R6Model: loaded", path=path)
            return self._is_trained
        except Exception as exc:
            logger.warning("R6Model: load failed", error=str(exc))
            return False
