"""
Dota2 Win Model — XGBoost with 6 Glicko-2 features.
...
MODEL_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", "..", "saved_models", "dota2_xgb.json"
)
...
class Dota2Model:
    ...
    def predict(self, game_state: Dict[str, Any]) -> float:
        ...
        try:
            ...
            logger.debug("Dota2Model: predict failed", error=str(exc))
            return 0.5

    def train(self, training_data: List[Dict[str, Any]]) -> bool:
        """Train on historical match data. Returns True if successful."""
        if len(training_data) < 50:
            logger.warning("Dota2Model: insufficient data", count=len(training_data))
            return False

        try:
            return self._train_sync(training_data)
        except Exception as exc:
            logger.error("Dota2Model: training failed", error=str(exc))
            return False
    ...
    def save(self, path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.info("Dota2Model: saved", path=path)
            return True
        except Exception as exc:
            logger.warning("Dota2Model: save failed", error=str(exc))
            return False
    ...
    def load(self, path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.info("Dota2Model: loaded", path=path)
            return self._is_trained
        except Exception as exc:
            logger.warning("Dota2Model: load failed", error=str(exc))
            return False
