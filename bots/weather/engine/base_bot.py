...
class BaseBot(ABC):
    def __init__(self, bot_name: str, base_engine: BaseEngine):
        ...
        try:
            from bots.weather.engine.base_engine.risk.bankroll_manager import BotBankrollManager
    ...
    async def store_pending_trade_signals(self, trade_id: str, market_id: str) -> None:
        ...
        try:
            ...
            if db is None:
                return
            from bots.weather.engine.base_engine.data.database import TradeSignal, _naive_utc
            from datetime import datetime, timezone
            row = TradeSignal(
        ...
        if getattr(settings, "USE_SCAN_JITTER", False):
            try:
                if self._strategic_timer is None:
                    from bots.weather.engine.base_engine.analysis.game_theory import StrategicTimer
    ...
    async def on_price_update(self, event: Dict[str, Any]) -> None:
        ...
        if _ws_recv_t > 0:
            ...
            try:
                from bots.weather.engine.base_engine.monitoring.metrics_collector import WS_SIGNAL_LATENCY
    ...
    async def apply_signal_enhancements(
        ...
        from bots.weather.engine.base_engine.learning.multiplier_aggregator import MultiplierAggregator
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    # (Option A, master 3324f2c; ported to wb/main 2026-06-08. hasattr
                    # guards make this safe even before the kill-switch objects gain the
                    # cached_* accessors — it just fails closed, today's behavior.)
                    _cached_allow = None
                    _mlks = getattr(self.base_engine, "multi_kill_switch", None)
