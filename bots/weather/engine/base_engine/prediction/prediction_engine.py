...
from bots.weather.engine.base_engine.learning.feature_engineering import FeatureEngineer
...
class PredictionEngine:
    ...
    async def _init_elevation_modules(self) -> None:
        ...
        # P3-05: Cross-market feature extractor
        try:
            from bots.weather.engine.base_engine.features.cross_market_features import CrossMarketFeatureExtractor
        ...
        # P3-06: Favorite-longshot calibrator
        try:
            from bots.weather.engine.base_engine.features.calibration import FavoriteLongshotCalibrator
        ...
        try:
            from bots.weather.engine.base_engine.features.calibration import HorizonBiasCalibrator
        ...
        try:
            from bots.weather.engine.base_engine.features.calibration import FocalTemperatureCalibrator
        ...
        # P5-03: LLM probability estimator
        try:
            from bots.weather.engine.base_engine.features.llm_probability import LLMProbabilityEstimator
        ...
        try:
            from bots.weather.engine.base_engine.prediction.chronos_forecaster import ChronosForecaster
    ...
    async def _train_models(self):
        ...
        if len(val_f) >= 10:
            ...
            from bots.weather.engine.base_engine.prediction.bias_detector import BiasDetector
    ...
    async def save_models_to_db(self) -> None:
        ...
        if not self.db.session_factory or not self.models:
            return
        import pickle
        from bots.weather.engine.base_engine.data.database import MLModel
    ...
    async def _prepare_training_data(self) -> tuple:
        ...
        from bots.weather.engine.base_engine.data.database import MarketPrice
        from bots.weather.engine.base_engine.learning.path_summary import (
        ...
        for i in range(len(df)):
            ...
            try:
                from bots.weather.engine.base_engine.learning.feature_engineering import compute_rsi, compute_bollinger_position, compute_atr_normalized
    ...
    async def _extract_features(
        ...
        from bots.weather.engine.base_engine.utils.validation import validate_price, validate_market_id
        ...
        try:
            ...
            from bots.weather.engine.base_engine.data.database import Market, User
            ...
            if _need_elite or _need_path or _need_regime or _need_fe or _need_l8:
                try:
                    ...
                    async with self.db.get_session() as _s:
                        ...
                        if _need_path:
                            try:
                                from bots.weather.engine.base_engine.learning.path_summary import get_path_summary, path_summary_to_feature_list
            ...
            if use_path:
                from bots.weather.engine.base_engine.learning.path_summary import path_summary_to_feature_list
            ...
            if use_regime:
                from bots.weather.engine.base_engine.learning.path_summary import regime_features_to_list, REGIME_DEFAULTS
