...
from bots.weather.engine.base_engine.data.data_ingestion import _infer_category
from bots.weather.engine.base_engine.data.resolution_observation import record_resolution_observation
...
async def run_resolution_backfill(
    ...
    **kwargs,
    ...
    from bots.weather.engine.base_engine.data.market_parser_v2 import MarketParserV2
    ...
    async with db.get_session() as session:
        ...
        try:
            ...
            try:
                from bots.weather.engine.config.settings import Settings as _settings_cls
        ...
        # LOGGED predictions on (never traded) are invisible to 2a/2b — their
        # `markets` rows are never resolution-checked, so prediction_log rows stay
        # unlabeled forever. See top-level base_engine/data/resolution_backfill.py
        # (the copy the ingestion service executes) for the full rationale.
        ...
        market_ids = paper_market_ids + other_ids + [
            _m for _m in pred_market_ids if _m not in _paper_set and _m not in set(other_ids)
        ]
    ...
    try:
        from bots.weather.engine.base_engine.data.trade_event_audit import audit_trade_events
