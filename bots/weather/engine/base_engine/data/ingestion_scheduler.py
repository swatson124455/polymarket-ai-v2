...
class IngestionScheduler:
    ...
    async def _run_ingestion(self) -> None:
        ...
        try:
            async with acquire_lock(db, "ingestion", timeout_seconds=60):
                ...
                if getattr(settings, "PIPELINE_CANARY_AFTER_INGESTION", True):
                    try:
                        from bots.weather.engine.base_engine.data.pipeline_canary import run_canary_after_markets, run_canary_after_prices
        ...
        if run_full and getattr(settings, "RUN_ORPHAN_CLEANUP_AFTER_INGESTION", False):
            try:
                from bots.weather.engine.base_engine.data.orphan_cleanup import run_orphan_cleanup
        ...
        if _health_due:
            try:
                from bots.weather.engine.base_engine.monitoring.health_runner import HealthRunner
    ...
    async def _do_mini_backfill(self, db, _now: datetime) -> None:
        ...
        # S172 7B Phase A3: backfill mirror_rejected_signals resolution.
        mrs_updated = 0
        try:
