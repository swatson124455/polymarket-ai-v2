...
from bots.weather.engine.base_engine.exceptions import DatabaseError
...
class Database:
        ...
        for market_data in markets:
            try:
                if getattr(settings, "PRE_INSERT_VALIDATION", True):
                    from bots.weather.engine.base_engine.utils.validation import validate_market_dict
        ...
        for price_data in prices:
            try:
                ...
                if row["market_id"] is None or row["token_id"] is None:
                    continue
                if getattr(settings, "PRE_INSERT_VALIDATION", True):
                    from bots.weather.engine.base_engine.utils.validation import validate_price_row
    ...
    async def bulk_insert_trades(self, trades: List[Dict[str, Any]]) -> None:
        ...
        for trade_data in trades:
            try:
                if getattr(settings, "PRE_INSERT_VALIDATION", True):
                    from bots.weather.engine.base_engine.utils.validation import validate_trade_dict
    ...
    async def get_latest_sync_completed_at(self) -> Optional[datetime]:
        ...
        async with self.get_session() as session:
            ...
            from bots.weather.engine.base_engine.data.database import SyncLog
            result = await session.execute(
    ...
    async def insert_prediction_log(
        ...
        try:
            async with self.get_session() as session:
                from bots.weather.engine.base_engine.data.database import PredictionLog
                log = PredictionLog(
                    market_id=market_id,
                    token_id=token_id,
    ...
    async def insert_mirror_rejected_signal(
        ...
        try:
            async with self.get_session() as session:
                from bots.weather.engine.base_engine.data.database import MirrorRejectedSignal
    ...
    async def check_database_status(self) -> Dict[str, Any]:
        ...
        async with self.get_session() as session:
            from sqlalchemy import select, func
            from bots.weather.engine.base_engine.data.database import Market, MarketPrice, Trade, DataQualityIssue
    ...
    async def verify_market_data_quality(self) -> Dict[str, Any]:
        ...
        async with self.get_session() as session:
            from sqlalchemy import select, func
            from bots.weather.engine.base_engine.data.database import Market, MarketPrice, Trade
