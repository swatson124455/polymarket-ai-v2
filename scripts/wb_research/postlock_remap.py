#!/usr/bin/env python3
"""S231 REMAPPED RUN: resolution stations per Gamma descriptions (Dallas=KDAL Love Field, Denver=KBKF Buckley, Houston=KHOU Hobby). Original run graded 3/11 cities against the WRONG airport; false-locks were 100%% artifact (0/268 at correct stations).
...
REMAP_STATION = {"dallas": "KDAL", "denver": "KBKF", "houston": "KHOU"}


def true_station(city, sid):
    return REMAP_STATION.get(city, sid)
...
def main():
    ...
    hourly = {}
    for sid3 in sorted({true_station(c, CITY[c].station_id)[1:] for (c, _, _) in fams}):
    ...
    for (city, mon, day), buckets in sorted(fams.items()):
        ...
        ld = datetime(2026, mon, day).date()
        obs = [(t, v) for t, v in hourly[true_station(city, st.station_id)[1:]]
