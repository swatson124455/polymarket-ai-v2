"""
WeatherBot standalone entry point (WB splinter — wb/main branch).

WHY THIS FILE EXISTS (S239 root cause):
  The shared `main.py` builds the TOP-LEVEL `base_engine.base_engine.BaseEngine` and
  injects it into every bot. WeatherBot's code, however, is bound to its *vendored silo*
  (`bots.weather.engine.*` — see bots/weather_bot.py:35-36). So at runtime WeatherBot's
  DB/order writes route through the top-level engine, NOT the silo it was written against
  — making every WB-only silo fix (S237 minute-bucket, S238 instrumentation, …) dead code.

  This launcher instantiates the SILO `BaseEngine` and runs ONLY WeatherBot with it, so the
  bot's code and its engine are the same tree. It is pointed at by the polymarket-weather
  systemd splinter drop-in (ExecStart … weather_main.py). Shared `main.py`, master, and the
  MB/EB splinters are never touched.

  This file is a DELIBERATE COPY of main.py's process lifecycle (preflight gate, watchdog,
  health endpoint, staggered start, graceful shutdown), reduced to a single bot, with only
  the engine/config imports swapped to the silo. It is intentionally self-contained (does
  NOT import from shared main.py) so WeatherBot owns its launcher end-to-end. If main.py's
  lifecycle helpers change, reconcile them here manually (see plan: standalone-engine §8
  "inverse divergence").

Startup flow (identical to main.py, one bot):
  1. Pre-flight health gate (DB + Redis ping)
  2. Initialize the SILO BaseEngine
  3. Start WeatherBot (if BOT_ENABLED_WEATHER)
  4. Watchdog monitors liveness; restarts a dead bot or exits for systemd restart
"""
import asyncio
import json
import signal
...
# (Silo copy of config/logging_setup.py)
from bots.weather.engine.config.logging_setup import configure_logging
configure_logging()

from bots.weather.engine.config.settings import settings

# Suppress noisy sklearn warnings BEFORE application imports (WeatherBot uses sklearn
# models — sklearn.utils.parallel emits UserWarning hundreds of times per scan cycle).
import warnings
    ...
    while True:
        ...
        if _db and _db.session_factory and _alerting:
            try:
                ...
                # NOTE (WB splinter): bot_heartbeats is a SHARED table — this WB-only process will see
                # heartbeat rows for MB/EB/etc. This full map + the disabled-flag gate below is what
                # suppresses cross-bot stale alerts (those flags are false in .env.weather). Keep the
                # map complete; an UNMAPPED stale bot would slip past the gate and alert.
                _bot_enabled_map = {
...
async def _health_server(bots_ref: dict, shutdown_event: asyncio.Event, port: int = 8765) -> None:
    ...
    Example:
        ...
        {"ok": true, "uptime_s": 142, "active_bots": ["WeatherBot"]}
...
async def main():
    logger.info("Starting WeatherBot standalone (silo engine)")
    ...
    try:
        ...
        # ── Initialize the SILO BaseEngine ─────────────────────────────
        ...
        # ── Instantiate WeatherBot (single-bot launcher) ───────────────
        if getattr(settings, "BOT_ENABLED_WEATHER", True):
            bots["WeatherBot"] = WeatherBot(base_engine)
        else:
            logger.warning("WeatherBot disabled via BOT_ENABLED_WEATHER — launcher idle")

        # ── Register bot with DegradationManager ───────────────────────
        # Feeds fleet-level tier recomputation. Must happen after the bot is
        # instantiated, before start() is called.
        if hasattr(base_engine, "degradation_manager") and base_engine.degradation_manager is not None:
            for bot_name in bots:
                try:
                    ...
                    # Attach machine so the scan loop can call record_health_ok/record_error
        ...
        # ── Start bot(s) with staggered delay ──────────────────────────
        # Stagger preserved from main.py for lifecycle parity (single bot here).
        ...
        logger.info("WeatherBot running -- press Ctrl+C to stop", active_bots=len(bots))
...
if __name__ == "__main__":
    ...
    try:
        ...
        import traceback
        print("\n============================================================")
        print("FATAL ERROR -- WeatherBot crashed during startup")
        print("============================================================")
        traceback.print_exc()
        print("============================================================")
        try:
