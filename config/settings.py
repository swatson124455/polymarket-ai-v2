...
class Settings(BaseSettings):
    ...
    ESPORTS_STALL_STARTUP_GRACE_S: float = float(os.getenv("ESPORTS_STALL_STARTUP_GRACE_S", "1200"))  # 2026-06-11 EB: stall watchdogs suppress force-exit until uptime > threshold+grace. Breaks the self-sustaining restart cycle (cold start → PgBouncer client_login_timeout kills nascent conns → scan #1 grinds 90s dead-conn timeouts past 900s → force-exit → new cold start). Genuine wedges still die at ~35min.
    SCAN_DEADLINE_S: float = float(os.getenv("SCAN_DEADLINE_S", "0"))  # 2026-06-13 EB: hard wall-clock deadline on one scan_and_trade(); 0=off (default, all services unchanged). esports sets 300 in .env.esports — bounds unbounded external I/O (e.g. PandaScore HTTP, upstream of the trade step) that DB timeouts can't reach. Must exceed DB command_timeout (90s) so it fires on HTTP not asyncpg; on expiry the scan aborts+retries (logged scan_deadline_exceeded), not counted as a failure.
    ...
    WEATHER_CAL_YES_MIN_SAMPLES: int = int(os.getenv("WEATHER_CAL_YES_MIN_SAMPLES", "100"))  # S159: 200→100; OOS Brier gate (S159) protects against overfitting at lower n
    ...
    MIRROR_MARKET_QUALITY_THRESHOLD: float = float(os.getenv("MIRROR_MARKET_QUALITY_THRESHOLD", "0.30"))  # S168: min market quality score (0=disabled)
    ...
    WEATHER_MAX_PER_GROUP_USD: float = float(os.getenv("WEATHER_MAX_PER_GROUP_USD", "20000"))  # S154: 10000→20000
    WEATHER_DAILY_LOSS_LIMIT: float = float(os.getenv("WEATHER_DAILY_LOSS_LIMIT", "10000"))
    WEATHER_MAX_CORRELATED_EXPOSURE: float = float(os.getenv("WEATHER_MAX_CORRELATED_EXPOSURE", "10000"))  # S154: 5000→10000
