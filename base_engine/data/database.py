    ...
    async def __aenter__(self):
        ...
        try:
            ...
            try:
                ...
                # Fix: close the session fully and raise. S223: the raise lands in the
                # outer except BaseException below, which releases the semaphore —
                # do NOT release here too (double-release would grow the limit).
                _CORRUPT_SIGS = (
            ...
            # exactly once, re-raise. The S235 corrupt path above unwinds session/
            # timeout itself (sets both to None) before raising, so it reaches here
            # with nothing left to unwind — the release below is its release too.
            try:
