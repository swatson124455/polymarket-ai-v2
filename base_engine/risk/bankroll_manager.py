...
_DEFAULT_BOT_CONFIGS: Dict[str, Dict[str, Any]] = {
    ...
    "MirrorBot":           {"capital": 3000, "kelly_fraction": 0.30, "max_bet_usd": 250, "max_daily_usd": 1500},
    ...
    "WeatherBot":          {"capital": 500,  "kelly_fraction": 0.25, "max_bet_usd": 50,  "max_daily_usd": 200},
...
class BotBankrollManager:
    ...
    async def get_bet_size(
        ...
        try:
            rm = getattr(self._db, "_risk_manager", None) if self._db else None
            if rm is None and self._gw is not None:
                rm = getattr(self._gw, "_risk_manager", None)
