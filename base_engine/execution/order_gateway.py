        ...
        if self._orderbook_tracker and token_id:
            try:
                ...
                if _book and not _book.get("error"):
                    ...
                    # S232: _shadow_best_ask / _shadow_best_bid must skip size-0 phantom
                    # top-of-book levels exactly as the walker does, or the S231 real-top-of-
                    # book anchor diverges from the walker's real-fill VWAP and false-rejects
                    # trades (production 2026-05-28, market 0xe7b4ced4: phantom best_ask=0.38
                    # vs walker VWAP=0.65 at slippage_cents=0.0). S245 R2: share the walker's
                    # filter via _parse_book_level so the two can never drift again — the
                    # original S232 bug was THIS filter being a hand-copy that diverged.
                    from base_engine.execution.paper_trading import _parse_book_level
                    _valid_ask: Optional[float] = None
                    for _lvl in _raw_asks:
                        _parsed = _parse_book_level(_lvl)
                        if _parsed is not None and (_valid_ask is None or _parsed[0] < _valid_ask):
                            _valid_ask = _parsed[0]
                    if _valid_ask is not None:
                        _shadow_best_ask = _valid_ask
                    _valid_bid: Optional[float] = None
                    for _lvl in _raw_bids:
                        _parsed = _parse_book_level(_lvl)
                        if _parsed is not None and (_valid_bid is None or _parsed[0] > _valid_bid):
                            _valid_bid = _parsed[0]
